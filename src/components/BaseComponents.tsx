...
import { View, StyleSheet, Text, ViewStyle, TextStyle, TouchableOpacity, Animated } from 'react-native';
...
export const TechCard: React.FC<CardProps> = ({ children, style, title, label }) => (
    ...
    {Array.from({ length: segments }).map((_, index) => (
      <View
        ...
        style={[
          ...
          index < filled && { shadowColor: color, shadowRadius: 4, elevation: 5 }
        ]}
      />
    ))}
  </View>
...
export const StatusDot = ({ color = COLORS.primary, size = 8, label }: { color?: string, size?: number, label?: string }) => (
  <View style={styles.dotContainer}>
    <View style={[styles.dot, { backgroundColor: color, width: size, height: size, borderRadius: size / 2 }]} />
    {label && <Text style={styles.dotLabel}>{label}</Text>}
  </View>
);
...
export const HXCoreAI = () => {
  const [opacity] = useState(new Animated.Value(0.3));
  const [message, setMessage] = useState('SYSTEM_SCANNING...');
  ...
  return (
    <Animated.View style={[styles.aiPanel, { opacity }]}>
      <MaterialCommunityIcons name="brain" size={16} color={COLORS.primary} />
      <Text style={styles.aiText}>HX-CORE: {message}</Text>
    </Animated.View>
  );
};
...
const styles = StyleSheet.create({
  ...
  segment: { height: '100%', borderRadius: 1 },
  dotContainer: { flexDirection: 'row', alignItems: 'center' },
  dot: { marginRight: 6 },
  dotLabel: { color: COLORS.textSecondary, fontSize: 10, ...FONTS.tech },
  header: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  headerTitle: { color: COLORS.textPrimary, fontSize: 16, fontWeight: 'bold', marginLeft: 10, ...FONTS.tech },
